...
*  **支持离线玩家托管：** 遭遇网络波动掉线也不必重开！系统可自动托管离线玩家，并在玩家重连时从房主权威状态恢复，降低旧状态导致的数据不同步风险。
*  **模组设置界面：** 可在游戏内配置玩家名称、玩家 ID、离线托管模式、兼容社区移动端移植版。
...
4. 玩家名称及玩家 ID 默认继承自 Steam 信息，您也可以在“模组设置”中自定义更改。
...
> **提示：** 模组配置会自动保存到 Godot 用户目录下的 `user://mods/DirectConnectIP/config.ini`。离线托管模式与兼容社区移动端移植版需要重启游戏后生效。
...
# 离线托管模式 (默认 true，需要重启游戏生效)
EnableOfflineTakeover=true
# 兼容社区移动端移植版 (默认 true，需要重启游戏生效)
